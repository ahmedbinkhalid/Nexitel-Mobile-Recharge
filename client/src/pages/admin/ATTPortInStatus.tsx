...
const SAMPLE_ATT_PORT_REQUESTS = [
  {
    id: "ATT-PORT-2025-001",
    phoneNumber: "+1 (555) 123-4567",
    fromCarrier: "Verizon",
    toService: "AT&T Prepaid",
    status: "Completed",
  ...
  {
    id: "ATT-PORT-2025-002", 
    phoneNumber: "+1 (555) 987-6543",
    fromCarrier: "T-Mobile",
    toService: "AT&T Postpaid",
    status: "In Progress",
...
export default function ATTPortInStatus() {
  console.log("ATTPortInStatus component is loading - this should be the clean port-in page!");
  const { user } = useAuth();
  const [searchTerm, setSearchTerm] = useState("");

  const filteredRequests = SAMPLE_ATT_PORT_REQUESTS.filter(request =>
      ...
      <div className="max-w-6xl mx-auto">
        <div className="mb-8">
          ...
          <div className="text-center">
            ...
            <p className="text-gray-600">Track your AT&T number porting requests</p>
          </div>
        </div>
                ...
                {request.status !== "Failed" && (
                  <div className="mt-6 pt-4 border-t">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center">
                        <div className={`w-3 h-3 rounded-full ${request.status === "Completed" || request.status === "In Progress" || request.status === "Pending Validation" ? "bg-blue-600" : "bg-gray-300"}`}></div>
                        <div className={`h-0.5 w-16 ${request.status === "Completed" || request.status === "In Progress" ? "bg-blue-600" : "bg-gray-300"}`}></div>
                        <div className={`w-3 h-3 rounded-full ${request.status === "Completed" || request.status === "In Progress" ? "bg-blue-600" : "bg-gray-300"}`}></div>
                        <div className={`h-0.5 w-16 ${request.status === "Completed" ? "bg-blue-600" : "bg-gray-300"}`}></div>
                        <div className={`w-3 h-3 rounded-full ${request.status === "Completed" ? "bg-blue-600" : "bg-gray-300"}`}></div>
                      </div>
        ...
        {filteredRequests.length === 0 && (
          <Card className="text-center py-12">
            <CardContent>
              ...
              <p className="text-gray-500">No AT&T port-in requests found matching your search.</p>
            </CardContent>
          </Card>
        )}
      </div>
